...
MWF.xApplication.CRM.PublicseasOpen = new Class({
    ...
    createForm:function(){
        ...
        var isManager = false;
        var fpHtml = "";
        this.actions.isManager(function (json) {
            if(json.type == "success"){
                debugger;
                var jsonObj = json.data;
                if(typeof(jsonObj.isManager)=="undefined"){
                }else{
                    isManager = jsonObj.isManager;
                }
            }
        }.bind(this));
        if(isManager){
            fpHtml = '<li class="el-dropdown-menu__item">分配</li><li class="el-dropdown-menu__item">删除</li>';
        }
        ...
        var moreHtml = '<ul class="el-dropdown-menu"><li class="el-dropdown-menu__item">领取</li>'+fpHtml+
            '<div class="popper__arrow"></div></ul>';
    ...
    createContentHtml: function (sectionId) {
        ...
        this.actions.getCustomerInfo(this.options.openId, function (json) {
            ...
            var tabPanel = "<div class='tabPanel'><div class='hit'>跟进记录</div><div style='width:30px'></div><div>基本信息</div><div style='width:30px'></div>"+
                    "<div>联系人</div><div style='width:30px'></div><div>相关团队</div><div style='width:30px'></div><div style='display:none'>商机</div><div style='display:none;width:30px'></div>"+
            ...
            var indexContentHtml = '<div class="log-cont"><div class="log-inner1"><div class="log-inner2"><div class="log-items">'+
                    ...
                    '<div class="empty-mask" style="display: none;margin-bottom:180px;"><div class="empty-content"><img src="/x_component_CRM/$Template/empty.png" class="empty-icon"> <p class="empty-text">没有找到数据</p></div></div></div>'+
    ...
    loadOptions: function() {
        ...
        var optionsHtml = '<div class="rc-cont">'+
            '<div class="empty-mask" style="display:none;margin-bottom:180px;"><div class="empty-content" style="margin-top:0px;"><img src="/x_component_CRM/$Template/empty.png" class="empty-icon"> <p class="empty-text">没有找到数据</p></div></div></div>'
    ...
    confirmReceiveCustomer: function () {
        this.actions.receiveCustomer(this.options.openId, function (json) {
            if(json.type == "success"){
                Showbo.Msg.alert('操作成功!', jQuery("#publicseas").click());
            }
        }.bind(this));
    ...
    deleteCustomer: function () {
        _self = this;
        Showbo.Msg.confirm('提示','确定要删除此公海客户吗?',function(){
        ...
        this.actions.deleteCustomer(this.options.openId, function (json) {
            if(json.type == "success"){
                ...
                if(this.isCurrentItem){
                    Showbo.Msg.alert('删除成功!',jQuery("#publicseas").click());
                }else{
                    Showbo.Msg.alert('删除成功!',jQuery("#publicseas").click());
                }

	...
	"load" : function() {
		...
		$.getJSON('../describe/describe.json?rd=' + Math.random(), function(json) {
			$.each(json.jaxrs, function(ji, j) {
				str += '<li xtype="menu" ' + 'style="margin-top: 15px;font-size:14px;font-weight:bold;"title="' +'" >' + j.name + ' <span style="font-style:italic">(' + j.description+ ')</span>';
				$.each(j.methods, function(mi, m) {
					str += '<ul><li xtype="li"  style="margin-top: 5px;margin-left:-24px;font-size:12px; font-weight:normal;line-height:20px" ><a id ="' + j.name + '_' + m.name + '" href="#"><b>' + m.name+'</b><br/><span style="color: #666666;">-'+ m.description + '</span>' + '</a></li></ul>';
				});
				str += '</li>'
			});
			str += '</ul>';

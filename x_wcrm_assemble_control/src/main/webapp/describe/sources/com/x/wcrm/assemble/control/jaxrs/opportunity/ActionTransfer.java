...
public class ActionTransfer extends BaseAction {
	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, String opportunityid, JsonElement jsonElement) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (StringUtils.isBlank(_transferType)) {
				throw new ExceptionOpportunityBaseMessage("转换类型不能为空。");
			} else {
				if (StringUtils.equalsIgnoreCase(_transferType, REMOVE_OWNER_ONLY)
					...
					throw new ExceptionOpportunityBaseMessage("转换类型不能识别。");
				}
			}
	...
	public static class Wo extends Opportunity {
		 ...
		 *
		 */
		private static final long serialVersionUID = 2072839326877157898L;

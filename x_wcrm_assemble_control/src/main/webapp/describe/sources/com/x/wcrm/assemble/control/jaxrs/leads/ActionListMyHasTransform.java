...
class ActionListMyHasTransform extends BaseAction {
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, Integer adjustPage, Integer adjustPageSize, JsonElement jsonElement)
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			List<Leads> os = leadsPermissionService.getList_MyDuty_And_SubNestedDuty_HasTransform(ThisApplication.context(), business,
					effectivePerson, adjustPage, adjustPageSize, wi.getKey(), wi.getOrderFieldName(), wi.getOrderType());
			List<Wo> wos = Wo.copier.copy(os);
			result.setData(wos);
			long count = leadsPermissionService.getList_MyDuty_And_SubNestedDuty__HasTransform_Count(ThisApplication.context(), business,

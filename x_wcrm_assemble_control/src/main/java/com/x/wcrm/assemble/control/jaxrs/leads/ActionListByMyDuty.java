...
class ActionListByMyDuty extends BaseAction {
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, Integer adjustPage, Integer adjustPageSize,
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			List<Leads> os = leadsPermissionService.getList_MyDuty(ThisApplication.context(), business, effectivePerson,
					adjustPage, adjustPageSize, wi.getKey(), wi.getOrderFieldName(), wi.getOrderType(), null, null);
			List<Wo> wos = Wo.copier.copy(os);
			result.setData(wos);
			long count = leadsPermissionService.getList_MyDuty_Count(ThisApplication.context(), business,
	...
	public static class Wo extends Leads {
		 ...
		 *
		 */
		private static final long serialVersionUID = 1585332369609656545L;

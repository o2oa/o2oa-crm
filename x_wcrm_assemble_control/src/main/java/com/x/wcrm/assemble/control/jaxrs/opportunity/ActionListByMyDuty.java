...
public class ActionListByMyDuty extends BaseAction {
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, Integer adjustPage, Integer adjustPageSize,
		...
		logger.info("所有我负责的商机。");
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			ActionResult<List<Wo>> result = new ActionResult<>();
			Wi wi = this.convertToWrapIn(jsonElement, Wi.class);
			Business business = new Business(emc);
			// List<Opportunity> os =
			// opportunityPermissionService.getList_MyDuty(ThisApplication.context(),
			// business, effectivePerson);
			List<Opportunity> os = opportunityPermissionService.getList_MyDuty(ThisApplication.context(), business,
			...
			result.setData(wos);
			long count = opportunityPermissionService.getList_MyDuty_Count(ThisApplication.context(), business,
	...
	public static class Wo extends WrapOutOpportunity {
		 ...
		 *
		 */
		private static final long serialVersionUID = 2636606599456505991L;
		// static WrapCopier<Opportunity, Wo> copier =
		// WrapCopierFactory.wo(Opportunity.class, Wo.class, null,

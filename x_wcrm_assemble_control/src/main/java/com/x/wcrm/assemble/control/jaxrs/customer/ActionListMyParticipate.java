...
class ActionListMyParticipate extends BaseAction {
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, Integer adjustPage, Integer adjustPageSize, JsonElement jsonElement)
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			//List<Customer> os = customerPermissionService.getList_MyParticipate(ThisApplication.context(), business, effectivePerson);
			List<Customer> os = customerPermissionService.getList_MyParticipate(ThisApplication.context(), business, effectivePerson, adjustPage,
					adjustPageSize, wi.getKey(), wi.getOrderFieldName(), wi.getOrderType());
			List<Wo> wos = Wo.copier.copy(os);
			result.setData(wos);
			long count = customerPermissionService.getList_MyParticipate_Count(ThisApplication.context(), business, effectivePerson, wi.getKey());
